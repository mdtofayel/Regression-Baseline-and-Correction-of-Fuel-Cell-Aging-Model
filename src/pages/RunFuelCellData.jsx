...
export default function RunAllPage() {
  ...
  const [targetColumn, setTargetColumn] = useState("UtestData");
  const [timestampColumn, setTimestampColumn] = useState("TimeData");
  const [targetDetectionMode, setTargetDetectionMode] = useState("force_utestdata");
  const [trainMode, setTrainMode] = useState("train-test");
  const [enableResidualCorrection, setEnableResidualCorrection] = useState(true);
  const [baselineModel, setBaselineModel] = useState("polynomial");
  ...
  const availableModelGroups = [
     {
      ...
      models: [
        { label: "baseline_polynomial", value: "baseline_polynomial" }
      ]
    },
  ...
  const RunFuelCelldata = async () => {
    ...
    try {
     const response = await axios.post("http://localhost:8080/runApi/fuelCellData", {
        files: selectedFiles,
        models: selectedMLModels,
        splits: testTrainPairs,
        target_column: targetColumn,
        timestamp_column: timestampColumn,
        target_detection_mode: targetDetectionMode,
        train_mode: trainMode,
        enable_residual_correction: enableResidualCorrection,
  ...
  return (
    <div className="p-6">
      ...
      <div className="border rounded h-[220px] overflow-y-auto mb-4">
        ...
        <div className="mb-6">
          <h4 className="font-semibold text-lg mb-3">⚙️ Fuel Cell Parameters</h4>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Timestamp Column</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Target Detection Mode</label>
              <input
                type="text"
                value={targetDetectionMode}
                onChange={(e) => setTargetDetectionMode(e.target.value)}
                className="border px-3 py-2 rounded w-full"
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700">Train Mode</label>
              <input
                type="text"
                value={trainMode}
            ...
            <div className="col-span-2 flex items-center gap-2">
              <input
                type="checkbox"
                checked={enableResidualCorrection}
                onChange={() => setEnableResidualCorrection(!enableResidualCorrection)}
                className="h-4 w-4"
              />
              <label className="text-sm font-medium text-gray-700">Enable Residual Correction</label>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700">Baseline Model</label>
              <input
                type="text"
                value={baselineModel}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Polynomial Degree</label>
              <input
                type="number"
                min={1}
